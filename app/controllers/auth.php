...
function auth_register()
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $enteredCode = trim($data['enteredCode']);
        ...
        if (!empty($confirmPassword) && ($confirmPassword !== $password)) $response['errors']['confirmPassword'] = "Passwords dont match!";
        if (!empty($password) && ($confirmPassword === $password) && strlen($password) < 8  ) {
            $response['errors']['password'] = "Password must be atleast 8 characters long "; 
            $response['errors']['confirmPassword'] = "Password must be atleast 8 characters long "; 
        }
        ...
        if (!empty($userType) && $userType !== 'user') {
            ...
            if (empty($enteredCode)) $response['errors']['code'] = "Enter the {$userType} code!";
            ...
            if (!empty($enteredCode)) {
                ...
                if (!$match) {
                    $response['errors']['code'] = "Incorrect {$userType} code! stop fighting a war you cant win!";
                }
            }
        }
        ...
        if (!empty($username) && !empty($usernameFromDb)) $response['errors']['username'] = "Username already exists";
        if (!empty($email) && !empty($emailFromDB)) $response['errors']['email'] = "Email already exists";
